...
def main():
    ...
    # args.question = '''six images come from one video:
    #                     (1) the first and second images are the first two frames where the green-boxed object appears,
    #                     (2) the third and fourth images are the last two frames of the the green-boxed object, and
    #                     (3) the fifth and sixth images are the first two frame where it disappears.

    #                     If boxes overlap, treat them as one object. The object in the bounding box is always the same one.

    #                     The first fourth images always contain the object inside the bounding box. If you think the object disappeared in the third or fourth image, it's also unnatural.

    #                     Task: Does the green-boxed object disappears naturally (occlusion, moving out of view, etc.) or unnaturally (sudden/non-physical disappearance)? You should answer based on the analyze of the three frames's object appearance change, visual continuity and interactions with surrounding vehicles and the environment.'''

    args.question = '''Given six frames (first appearance in I1–I2, last appearance in I3, then disappear in I4–I6) of the same green-boxed object, \
