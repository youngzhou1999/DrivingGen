...
def calc_metrics_(metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int):
    dnnlib.util.Logger(should_flush=True)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # if args.num_gpus == 1:
            metrics = subprocess_fn(rank=0, args=args, temp_dir=temp_dir)
        # else:
            # torch.multiprocessing.spawn(fn=subprocess_fn, args=(args, temp_dir), nprocs=args.num_gpus)
